...
int check_edge_validity(int i, int j, int visited[]) {
    if(i == j) return 0;
    if(visited[i] == 0 && visited[j] == 0) return 0;
    else if(visited[i] == 1 && visited[j] == 1) return 0;
    return 1;
}

void finding_mst(int *mtrx, int num_of_vrtx) {
    int i, j, visited[num_of_vrtx], min, x = -1, y = -1, minCost = 0, edge_count = 0;
    for(i = 0; i < num_of_vrtx; i++) visited[i] = 0;
    
    visited[0] = 1;
    while (edge_count < num_of_vrtx - 1) {
        min = 9999, x = -1, y = -1;
        for(i = 0; i < num_of_vrtx; i++) {
            for(j = 0; j < num_of_vrtx; j++) {
